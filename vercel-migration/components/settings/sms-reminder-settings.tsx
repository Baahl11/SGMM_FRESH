        ...
        <Card className="border-orange-200 bg-orange-50">
          <CardHeader>
            <CardTitle className="text-sm flex items-center gap-2 text-orange-900">
              ...
              Configuración Requerida para {SMS_PROVIDERS[config.provider].name}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="text-sm text-orange-900">
              <p className="font-medium mb-2">Variables de entorno necesarias:</p>
              <div className="bg-orange-100 rounded p-3 font-mono text-xs space-y-1">
                {SMS_PROVIDERS[config.provider].requires.map((req) => (
                  <div key={req} className="flex items-center gap-2">
                    <CheckCircle2 className="h-3 w-3" />
                    <span>{`${config.provider.toUpperCase()}_${req.toUpperCase()}`}</span>
                  </div>
                ))}
              </div>
              <p className="mt-3 text-xs">
                Configura estas variables en tu archivo .env.local para habilitar el envío real de SMS.
              </p>
            </div>
          </CardContent>
        </Card>
