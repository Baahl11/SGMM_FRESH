...
export async function POST(
  ...
  try {
    ...
    // 3. Validate slot with smart calendar system (checks bookings AND appointments)
    ...
    if (!validation.valid) {
      return NextResponse.json(
        { 
          error: validation.error || 'Este horario ya no está disponible',
          code: 'SLOT_UNAVAILABLE',
          suggestion: validation.suggestion // Next available slot
        },
        { status: 409 }
      )
    }
    ...
    try {
      ...
      await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/notifications/send`, {
        ...
        body: JSON.stringify({
          ...
          event_type: settings.auto_confirm ? 'booking_confirmed' : 'booking_received',
          send_email: true,
          send_whatsapp: true,
        }),
      }).catch(err => {
