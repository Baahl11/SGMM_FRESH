...
function getMigrationFiles() {
  const fromEnv = (process.env.HOTFIX_MIGRATIONS || '')
    .split(',')
    .map((v) => v.trim())
    .filter(Boolean);

  const files = fromEnv.length > 0 ? fromEnv : defaultMigrationFiles;
...
async function main() {
  const migrationBundle = getMigrationFiles();
  ...
  console.log(
    JSON.stringify(
      {
        projectRef,
        migrationFiles: migrationBundle.map((m) => m.file),
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      console.log('HOTFIX_BUNDLE_APPLIED', JSON.stringify({ ...sanitizeCandidate(candidate), migrations: migrationBundle.map((m) => m.file) }));
      await client.end();
      process.exit(0);
    } catch (err) {
