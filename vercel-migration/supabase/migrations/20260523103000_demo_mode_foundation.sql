-- Migration: Demo mode foundation (account flag + integration simulation audit)
-- Date: 2026-05-23
-- Purpose: Allow demo accounts to simulate external integrations safely.
...
COMMENT ON TABLE demo_mode_config IS 'Per-account demo mode configuration and integration simulation policies.';
COMMENT ON COLUMN demo_mode_config.integrations IS 'JSONB map per integration: {enabled:boolean,mock:boolean}. Demo accounts simulate by default.';
...
CREATE INDEX IF NOT EXISTS idx_demo_mode_config_demo_expires_at
  ON demo_mode_config(demo_expires_at)
...
COMMENT ON TABLE demo_audit_log IS 'Audit trail for simulated actions executed in demo mode.';

CREATE INDEX IF NOT EXISTS idx_demo_audit_log_user_created
  ON demo_audit_log(user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_demo_audit_log_integration
  ON demo_audit_log(integration, created_at DESC);
