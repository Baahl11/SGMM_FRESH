...
export function calcularComisionTarjeta(
  ...
  // Solo agregar IVA para ciertas tarjetas (como OpenPay que ya estaba configurado)
  // Para las nuevas configuraciones, las tasas ya incluyen todo
...
export function calcularGananciaNeta(
  ...
  if (metodoPago === 'tarjeta') {
    if (!tipoTarjeta) {
      // Si no hay tipo de tarjeta seleccionado, devolver valores por defecto
      return {
        tasa: 0,
        comision: 0,
        ganancia: gananciaBase,
  ...
  // Para cualquier otro método de pago, devolver sin comisiones
  return {
    tasa: 0,
    comision: 0,
    ganancia: gananciaBase,
