...
const getApiUrl = () => {
  // USAR rutas API estáticas para MSI compatibility en lugar del proxy
  return "/api";
};
...
function toApiUrl(input: string): string {
  ...
  // SIMPLIFICADO: Solo manejar casos básicos para API estáticas
  console.log('🔍 toApiUrl input:', input);

  // 1) Si ya es una URL API estática correcta, déjala tal como está
  if (url.startsWith('/api/') && !url.includes('proxy')) {
    console.log('🔍 toApiUrl: Ya es API URL estática, mantener:', url);
    return url;
  }

  // 2) Si es una URL absoluta al backend, convertir a API estática
  if (/^https?:\/\/(localhost|127\.0\.0\.1):8000/.test(url)) {
    url = url.replace(/^https?:\/\/(localhost|127\.0\.0\.1):8000/, '');
    console.log('🔍 toApiUrl: Convertir URL absoluta backend:', url);
  }

  // 3) Si empieza con /api/proxy/, convertir a API estática
  if (url.startsWith('/api/proxy/')) {
    url = url.replace('/api/proxy/', '/api/');
    console.log('🔍 toApiUrl: Convertir proxy a API estática:', url);
    return url;
  }

  // 4) Si es una ruta relativa de endpoint, añadir API
  if (url.startsWith('/')) {
    url = '/api' + url;
    console.log('🔍 toApiUrl: Añadir API a ruta absoluta:', url);
    return url;
  }

  // 5) Si no empieza con /, es relativa, añadir api/
  url = '/api/' + url;
  console.log('🔍 toApiUrl: Convertir relativa a API estática:', url);
...
class ApiService {
  ...
  static async uploadPatientImage(patientId: number, file: File): Promise<ApiResponse<any>> {
    try {
      ...
      const response = await fetch(`${API_URL}/api/patients/${patientId}/upload-image`, {
        method: "POST",
        headers: {
  ...
  static async getPatientImages(patientId: number): Promise<ApiResponse<any>> {
    try {
      const response = await this.fetchWithAuth(`/api/patients/${patientId}/images`);
  ...
  static async updateImageComment(patientId: number, imageName: string, comment: string): Promise<ApiResponse<any>> {
    try {
      const response = await this.fetchWithAuth(`/api/patients/${patientId}/images/${encodeURIComponent(imageName)}/comment`, {
        method: "PUT",
    ...
    try {
      const response = await this.fetchWithAuth(`/api/patients/${patientId}/images?image=${encodeURIComponent(imageName)}`, {
        method: "DELETE",
      });
  ...
  static async getRecords(patientId?: number): Promise<ApiResponse<any[]>> {
    try {
      ...
      const response = await this.fetchWithAuth(`/api/proxy/records${queryParams}`);
    ...
    try {
      const endpoint = patientId ? `/api/proxy/records/with-names/?patient_id=${patientId}` : "/api/proxy/records/with-names/";
  ...
  static async createMultipleRecord(recordData: any): Promise<ApiResponse<any>> {
    try {
      const response = await this.fetchWithAuth("/api/proxy/records/multiple/", {
        method: "POST",
  ...
  static async getInventoryItems(): Promise<ApiResponse<any[]>> {
    try {
      const response = await this.fetchWithAuth("/inventory/");
  ...
  static async getTreatmentInventory(treatmentId: number): Promise<ApiResponse<any[]>> {
    try {
      const response = await this.fetchWithAuth(`/treatments/${treatmentId}/inventory`);
  ...
  static async removeTreatmentInventoryItem(treatmentId: number, itemId: number): Promise<ApiResponse<any>> {
    try {
      const response = await this.fetchWithAuth(`/treatments/${treatmentId}/inventory/${itemId}`, {
        method: "DELETE",
      });
  ...
  static async queryAI(query: string, includeHistory: boolean = true): Promise<ApiResponse<any>> {
    try {
      const response = await this.fetchWithAuth("/ai/query/", {
        method: "POST",
        body: JSON.stringify({
          query,
  ...
  static async getAppointments(params?: {
    ...
    try {
      ...
      const url = `/appointments/${searchParams.toString() ? `?${searchParams}` : ''}`;
      const response = await this.fetchWithAuth(url);
  ...
  static async getAppointmentsWithNames(params?: {
    ...
    try {
      ...
      const url = `/appointments/with-names/${searchParams.toString() ? `?${searchParams}` : ''}`;
      const response = await this.fetchWithAuth(url);
  ...
  static async updateAppointment(id: number, appointmentData: any): Promise<ApiResponse<any>> {
    try {
      const response = await this.fetchWithAuth(`/appointments/${id}`, {
        method: "PUT",
        body: JSON.stringify(appointmentData),
      });
  ...
  static async getAIHelp(): Promise<ApiResponse<any>> {
    try {
      const response = await this.fetchWithAuth("/ai/help/");
      if (!response.ok) {
  ...
  static async updateBillingSettings(settingsId: number, settings: any): Promise<ApiResponse<any>> {
    try {
      const response = await this.fetchWithAuth(`/billing/settings/${settingsId}/`, {
        method: "PUT",
        body: JSON.stringify(settings),
      });
  ...
  static async testEmailConfiguration(): Promise<ApiResponse<any>> {
    try {
      const response = await this.fetchWithAuth("/billing/test-email-config/", {
        method: "POST",
      });
  ...
  // Create invoice from records
  static async createInvoiceFromRecords(recordIds: number[]): Promise<ApiResponse<any>> {
    try {
      const response = await fetch("/api/proxy/billing/create-invoice", {
        method: "POST",
        headers: this.getAuthHeaders(),
  ...
  static async createInvoice(invoiceData: any): Promise<ApiResponse<any>> {
    try {
      const response = await fetch("/api/proxy/billing/invoices", {
        method: "POST",
        headers: this.getAuthHeaders(),
  ...
  static async stampInvoice(invoiceId: number): Promise<ApiResponse<any>> {
    try {
      const response = await fetch(`/api/proxy/billing/invoices/${invoiceId}/stamp`, {
        method: "POST",
        headers: this.getAuthHeaders(),
      });
  ...
  static async cancelInvoice(invoiceId: number, reason: string): Promise<ApiResponse<any>> {
    try {
      const response = await fetch(`/api/proxy/billing/invoices/${invoiceId}/cancel`, {
        method: "POST",
        headers: this.getAuthHeaders(),
  ...
  static async downloadInvoiceXML(invoiceId: number): Promise<ApiResponse<Blob>> {
    try {
      const response = await fetch(`/api/proxy/billing/invoices/${invoiceId}/xml`, {
        method: "GET",
        headers: {
  ...
  static async downloadInvoicePDF(invoiceId: number): Promise<ApiResponse<Blob>> {
    try {
      const response = await fetch(`/api/proxy/billing/invoices/${invoiceId}/pdf`, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${AuthService.getToken()}`,
        },
  ...
  static async sendInvoiceEmail(invoiceId: number, email: string): Promise<ApiResponse<any>> {
    try {
      const response = await fetch(`/api/proxy/billing/invoices/${invoiceId}/send-email`, {
        method: "POST",
        headers: this.getAuthHeaders(),
