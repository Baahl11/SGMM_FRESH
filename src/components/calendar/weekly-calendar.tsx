...
export default function WeeklyCalendar({ className = "" }: WeeklyCalendarProps) {
  ...
  const loadAppointments = async () => {
    try {
      ...
      if (response.data) {
        // Set week start to beginning of day (00:00:00)
        const weekStartTime = new Date(currentWeekStart);
        weekStartTime.setHours(0, 0, 0, 0);
        
        // Set week end to end of day (23:59:59)
        const weekEndTime = new Date(currentWeekStart);
        weekEndTime.setDate(weekEndTime.getDate() + 6);
        weekEndTime.setHours(23, 59, 59, 999);
        
        console.log('Filtering appointments for week:', {
          weekStart: weekStartTime.toISOString(),
          weekEnd: weekEndTime.toISOString()
        });
        
        const weeklyAppointments = response.data.filter((record: any) => {
          // Convert record date to local time for comparison
          const recordDate = new Date(record.fecha);
          const isAppointment = record.monto_pagado === 0;
          const isInWeek = recordDate >= weekStartTime && recordDate <= weekEndTime;
          ...
          return isAppointment && isInWeek;
        });

        console.log(`Found ${weeklyAppointments.length} appointments for current week`);
