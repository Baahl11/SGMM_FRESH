...
export default function MessagingPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setConfig({
          ...
          email_advance_hours: data.email_advance_hours || 24,
          whatsapp_advance_hours: data.whatsapp_advance_hours || 2,
          email_template: data.email_template || 'Estimado/a {nombre_paciente}, le recordamos que tiene una cita médica programada para el {fecha_cita} a las {hora_cita}. Consultorio Médico.',
          whatsapp_template: data.whatsapp_template || 'Hola {nombre_paciente}! 👋 Recordatorio: Tienes cita médica el {fecha_cita} a las {hora_cita}. Te esperamos 🏥',
  ...
  const testWhatsAppNotification = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success('WhatsApp Business de prueba enviado exitosamente');
      } else {
        const errorData = await response.json();
        toast.error(errorData.error || 'Error al enviar WhatsApp Business de prueba');
      }
    } catch (error) {
      toast.error('Error de conexión');
    } finally {
