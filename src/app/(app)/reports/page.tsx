  ...
  useEffect(() => {
    ...
    try {
      ...
      const [patientsResponse, treatmentsResponse, recordsResponse, gastosFijosResponse] = await Promise.all([
        ...
        // Llamar directamente al backend para records
        fetch('/api/proxy/records')
          .then(res => res.json())
          .then(data => ({ data: data.data || data, error: undefined }))
          .catch(err => ({ data: undefined, error: err.message })),
        // Llamar directamente al backend para gastos fijos
        fetch('/api/proxy/gastos-fijos')
          .then(res => res.json())
          .then(data => ({ data: data.data || data, error: undefined }))
          .catch(err => ({ data: undefined, error: err.message }))
      ]);      if (patientsResponse.error || treatmentsResponse.error || 
          ('error' in recordsResponse && recordsResponse.error) || 
          ('error' in gastosFijosResponse && gastosFijosResponse.error)) {
        const errorDetails = {
          patientsError: patientsResponse.error,
          treatmentsError: treatmentsResponse.error,
          recordsError: 'error' in recordsResponse ? recordsResponse.error : undefined,
          gastosFijosError: 'error' in gastosFijosResponse ? gastosFijosResponse.error : undefined
        };
      ...
      const records = ('data' in recordsResponse && recordsResponse.data) ? recordsResponse.data : [];
      const gastosFijos = ('data' in gastosFijosResponse && gastosFijosResponse.data) ? gastosFijosResponse.data : [];

      console.log("Data received:", {
        patients: patients.length,
        treatments: treatments.length, 
        records: records.length,
        gastosFijos: gastosFijos.length,
        gastosFijosStructure: gastosFijos[0] || null,
        recordsStructure: records[0] || null,
    ...
    // Función helper para crear datos diarios - FIXED timezone handling
    const createDailyData = (days: number) => {
      ...
      console.log('📅 DEBUG: Date range creation', {
        today: today.toISOString(),
        todayLocal: today.toLocaleDateString(),
        todayManual: `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`,
      ...
      for (let i = days - 1; i >= 0; i--) {
        ...
        // Debug each date in the range
        if (i <= 2) { // Log last 3 days for debugging
          console.log(`📅 Date range item i=${i}:`, {
            dateStr,
            dayName: date.toLocaleDateString('es-MX', { weekday: 'long', day: 'numeric', month: 'short' }),
            isToday: i === 0
          });
        }
      }

      // Procesar registros por día - con logging detallado y manejo robusto de fechas
      console.log('🔍 DEBUG: Procesando registros para reportes', {
        totalRecords: records.length,
        days: days,
        dateRange: Object.keys(dailyData),
        dateRangeDetailed: Object.keys(dailyData).map(dateStr => {
          const [year, month, day] = dateStr.split('-').map(Number);
          const dateObj = new Date(year, month - 1, day);
          return {
            key: dateStr,
            display: dateObj.toLocaleDateString('es-MX', { 
              weekday: 'long', 
              month: 'long', 
              day: 'numeric' 
            })
        ...
        firstRecords: records.slice(0, 3).map(r => ({
          id: r.id,
          fecha: r.fecha,
      ...
      records.forEach(record => {
        try {
          ...
          // Robust date parsing - same logic as today calculation
          let recordDate;
          ...
          console.log('📅 Processing record:', {
            ...
            isInDailyData: !!dailyData[recordDate]
          });
          ...
          if (dailyData[recordDate]) {
            ...
            console.log('💰 Updated daily data for', recordDate, {
              revenue: dailyData[recordDate].revenue,
              costs: dailyData[recordDate].costs,
              profit: dailyData[recordDate].profit
            });
          } else {
            console.log('⚠️ Record date not in daily data range:', recordDate);
          }
        } catch (error) {
      ...
      return Object.entries(dailyData).map(([date, data]) => {
        // FIXED: Avoid timezone issues when formatting date labels
        // Parse the date string manually to avoid UTC interpretation
        const [year, month, day] = date.split('-').map(Number);
        const dateObj = new Date(year, month - 1, day); // Local date construction
        
        console.log('📅 Formatting date for display:', {
          originalDateString: date,
          parsedDate: dateObj.toISOString().split('T')[0],
          formattedDisplay: dateObj.toLocaleDateString('es-MX', { 
            month: 'short', 
            day: 'numeric',
    ...
    const daily90Revenue = createDailyData(90);// Calcular ingresos de hoy con gastos fijos - FIXED timezone handling
    ...
    const paymentMethods = Object.entries(paymentMethodCounts).map(([name, value], index) => {
      ...
      if (name === 'tarjeta_credito' || name === 'Tarjeta_credito') {
        formattedName = 'Tarjeta de crédito';
      } else if (name === 'tarjeta' || name === 'Tarjeta') {
        formattedName = 'Tarjeta de débito';
      } else if (name === 'transferencia') {
        formattedName = 'Transferencia';
      } else if (name === 'efectivo') {
        formattedName = 'Efectivo';
      } else {
        // Formateo genérico: reemplazar guiones bajos con espacios y capitalizar
        formattedName = name.replace(/_/g, ' ').charAt(0).toUpperCase() + name.replace(/_/g, ' ').slice(1);
      }
    ...
    const sortedTopTreatments = safeSort(topTreatments, (a: any, b: any) => b.revenue - a.revenue)
      .slice(0, 5);

    // Actividad de pacientes (últimos 30 días) - FIX: usar fecha_nacimiento
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const dateStr = date.toISOString().split('T')[0];
        ...
        // FIX: Como created_at es null, simulamos distribución o usamos otro criterio
        // Para mostrar datos, vamos a mostrar el total de pacientes distribuido
        return false; // Por ahora mostramos 0 nuevos hasta implementar correctamente
      }).length;
      
      const totalPatients = patients.length; // Mostrar total actual de pacientes
                ...
                {reportData.paymentMethods.map((method, index) => (
                  <div key={index} className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      {method.name === 'Tarjeta de crédito' && <CreditCard className="h-4 w-4" />}
                      {method.name === 'Tarjeta de débito' && <CreditCard className="h-4 w-4" />}
