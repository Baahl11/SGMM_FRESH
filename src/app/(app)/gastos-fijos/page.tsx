...
export default function GastosFijosPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let response;
      if (editingGasto) {
        response = await fetch(`/api/gastos-fijos/${editingGasto.id}/`, {
          method: 'PUT',
          headers: {
        ...
        response = await fetch('/api/gastos-fijos/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      if (!response.ok) {
        throw new Error('Error al guardar gasto fijo');
      }
