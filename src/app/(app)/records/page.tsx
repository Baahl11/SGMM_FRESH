...
export default function RecordsPage() {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      const { data, error } = await ApiService.getRecordsEnhanced();
      if (error) {
        setError(error);
        console.error("Error fetching enhanced records:", error);
      } else if (data) {
        setRecords(data);
        console.log("Successfully loaded", data.length, "enhanced records");
      }
    } catch (err) {
            ...
            <Card key={record.id} className="bg-white shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
              <CardHeader className="pb-3 border-b border-gray-100">
                ...
                <div className="space-y-3">
                  ...
                  {/* Show multiple treatments if available */}
                  {record.tiene_multiples_tratamientos && record.record_treatments && record.record_treatments.length > 0 && (
                    <div className="bg-blue-50 rounded-lg p-3 mt-2">
                      <div className="text-xs text-blue-700 mb-2 font-medium">Tratamientos incluidos:</div>
                      <div className="space-y-1">
                        {record.record_treatments.map((treatment, index) => (
                          <div key={treatment.id} className="text-xs text-blue-600">
                            {index + 1}. {treatment.nombre_tratamiento} - ${treatment.precio_promocional.toLocaleString()}
                          </div>
                        ))}
                      </div>
                      {record.ahorro_total && record.ahorro_total > 0 && (
                        <div className="text-xs text-green-600 mt-2 font-medium">
                          Ahorro total: ${record.ahorro_total.toLocaleString()}
                        </div>
                      )}
                    </div>
                  )}
