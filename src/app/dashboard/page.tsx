...
export default function DashboardPage() {  const [stats, setStats] = useState<DashboardStats>({
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [patientsResponse, treatmentsResponse, recordsResponse, dashboardStatsResponse, inventoryHealthResponse] = await Promise.all([
        ApiService.getPatients(),        ApiService.getTreatments(),
        ApiService.getRecordsWithNames(),
        ApiService.getDashboardStats(),
        authenticatedFetch('/api/inventory/health').then(res => {
          console.log('Dashboard: Inventory health response status:', res.status);
          return res.json();
        }).catch(error => {
          console.error('Dashboard: Error fetching inventory health:', error);
          return null;
        })
      ]);
      ...
      const billedPercentage = totalCalculatedRevenue > 0 ? (billedRevenue / totalCalculatedRevenue) * 100 : 0;      setStats({
        ...
        paymentMethods,        recentRecords: processedRecords
      });

      // Actualizar estado del inventario
      if (inventoryHealthResponse && !inventoryHealthResponse.error) {
        console.log('✅ Dashboard: Inventory health data received:', inventoryHealthResponse);
        setInventoryHealth(inventoryHealthResponse);
      } else {
        console.error('❌ Dashboard: Inventory health error:', inventoryHealthResponse);
      }
      ...
      <Card className="bg-white shadow-sm border border-gray-200">
        <CardHeader className="border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-lg font-semibold text-gray-900">Calendario Semanal de Citas</CardTitle>
          </div>
        </CardHeader>
        <CardContent className="p-6">
          <WeeklyCalendar />
