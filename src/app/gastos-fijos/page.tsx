...
export default function GastosFijosPage() {
  ...
  const openEditDialog = (gasto: GastoFijo) => {
    ...
    setFormData({
      ...
      fecha_inicio: gasto.fecha_inicio.split('T')[0], // Extract date part
      notas: gasto.notas || ""
    });
    ...
    <div className="space-y-8 p-6 bg-gradient-to-br from-blue-50/50 to-indigo-50/50 min-h-screen">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{gastosFijos.filter(g => g.activo).length}</div>
              ...
              {gastosFijos.map((gasto) => (
                <div
                  ...
                  className={`flex items-center justify-between p-4 border rounded-lg ${
                    !gasto.activo ? "opacity-50 bg-gray-50" : ""
                  }`}
                >
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground mt-1">
                      ...
                      {" • "}
                      <span className="capitalize">{gasto.frecuencia}</span>
                      {" • "}
                      <span>Desde: {new Date(gasto.fecha_inicio).toLocaleDateString('es-MX')}</span>
                    </div>
                    {gasto.notas && (
